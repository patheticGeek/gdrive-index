...
const { CLIENT_ID, CLIENT_SECRET, TOKEN, AUTH_CODE, PARENT_FOLDER, SCOPE } = dev ? require("../config.json") : process.env;
const SCOPES = JSON.parse(SCOPE);

class Drive {
  constructor() {
    if (!CLIENT_ID || !CLIENT_SECRET) {
      this.logger("CLIENT_ID, CLIENT_SECRET enviorment variables not found");
      return;
    }
    ...
    this.authorize()
      ...
      .catch((e) => this.logger(`Auth error: ${e.message}`));
  }
  ...
  async authorize() {
    if (TOKEN) {
      ...
      this.logger(`Get AUTH_CODE enviorment variable by visiting this url: \n${authUrl}\n`);
    }
  }
